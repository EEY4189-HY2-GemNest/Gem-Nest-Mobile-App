...
class AuctionScreenOptimized extends StatefulWidget {
  const AuctionScreenOptimized({super.key});

  @override
  State<AuctionScreenOptimized> createState() => _AuctionScreenOptimizedState();
}

class _AuctionScreenOptimizedState extends State<AuctionScreenOptimized> {
  ...
  Widget _buildAuctionsList() {
    return StreamBuilder<List<Auction>>(
      stream: _repository.getFilteredAuctions(
      ...
      builder: (context, snapshot) {
        ...
        // Apply search filter in-memory - O(n)
        if (_searchQuery.isNotEmpty) {
          auctions = _repository.searchAuctions(_searchQuery, auctions);
        }
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return Padding(
              ...
              child: OptimizedAuctionItemCard(
                auction: auction,
...
class OptimizedAuctionItemCard extends StatefulWidget {
  final Auction auction;
  final AuctionRepository repository;

  const OptimizedAuctionItemCard({
    super.key,
  ...
  @override
  State<OptimizedAuctionItemCard> createState() =>
      _OptimizedAuctionItemCardState();
}

class _OptimizedAuctionItemCardState extends State<OptimizedAuctionItemCard>
  ...
  void _setupRealtimeListener() {
    _auctionSubscription = FirebaseFirestore.instance
      ...
      if (snapshot.exists) {
        ...
        setState(() {
          _currentBid = (data['currentBid'] as num?)?.toDouble() ?? _currentBid;
          _winningUserId = data['winningUserId'];
        });
        if (_currentBid > widget.auction.currentBid) {
  ...
  Future<void> _placeBid() async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => _buildBidConfirmDialog(enteredBid),
    );

    if (confirm ?? false) {
      setState(() => _isLoading = true);
      try {
        final success = await widget.repository.placeBid(
        ...
        if (success) {
          ...
          _showSnackBar('Error placing bid');
        }
      } catch (e) {
        setState(() => _isLoading = false);
  ...
  Widget _buildBidConfirmDialog(double enteredBid) {
    return Dialog(
